...
form <- aaply(form, 1, function(x) gsub(pattern = '[^[:alnum:] ]', '',
                                        x, perl = TRUE))
known <- gov$formation[gov$environment1 != '']
...
kk <- cbind(kk$formation, kk$lithology1, kk$lithology2, kk$environment)
...
got <- data.frame(cbind(got$formation, got$lithology1, got$lithology2, got$environment1, got$environment2), stringsAsFactors = FALSE)

# ok, make a table that has old and new next to each other
improve <- cbind(kk, got[, -1])
names(improve) <- c('formation', 'PBDB lithology 1', 'PBDB lithology 2',
                    'PBDB paleoenvironment', 'my lithology 1', 'my lithology 2',
                    'my paleoenvironment 1', 'my paleoenvironment 2')
